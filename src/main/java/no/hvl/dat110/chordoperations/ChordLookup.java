...
public class ChordLookup {
	...
	public NodeInterface findSuccessor(BigInteger key) throws RemoteException {
		...
		// check if key is in (nodeID, succID]
		boolean cond = Util.checkInterval(key, nodeID.add(BigInteger.ONE), succ.getNodeID());
	
		if (cond) {
			return succ;
		}
	
		// otherwise find highest predecessor and continue recursively
		NodeInterface highest_pred = findHighestPredecessor(key);
	
		// avoid infinite recursion if highest predecessor is this node
	...
	private NodeInterface findHighestPredecessor(BigInteger ID) throws RemoteException {
		...
		for (int i = fingers.size() - 1; i >= 0; i--) {
			...
			// check if finger is in (nodeID, ID)
